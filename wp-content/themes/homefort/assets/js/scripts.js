...
function initCollapseMenu() {
  ...
  navbar.on("click", ".menu-item-type-custom > a", function (e) {
    ...
    if (
      jQuery(this).attr("href") != "#pll_switcher" &&
      typeof jQuery(this).attr("href") !== "undefined" &&
      jQuery(this).attr("href") !== "#" &&
      jQuery(this).attr("href").indexOf("#") != -1 &&
      ((el.closest("li").hasClass("menu-item-has-children") &&
        e.target.tagName != "A") ||
        (!el.closest("li").hasClass("menu-item-has-children") &&
          e.target.tagName == "A"))
    ) {
...
function checkNavbar() {
  ...
  if (winWidth < navbar_mobile_width) {
    ...
    navbar_wrapper
      .addClass("lte-navwrapper-mobile")
      .removeClass("lte-navwrapper-desktop");
  } else {
    navbar.addClass("navbar-desktop").removeClass("navbar-mobile");
    navbar_wrapper
      .addClass("lte-navwrapper-desktop")
      .removeClass("lte-navwrapper-mobile");
  }
  ...
  jQuery(
    "#lte-nav-wrapper.navbar-layout-transparent + .lte-page-header, #lte-nav-wrapper.navbar-layout-transparent + .main-wrapper"
  ).css("margin-top", "-" + navbar_wrapper.height() + "px");
  ...
  if (jQuery(".lte-mouse-move-object").length) {
    jQuery(".lte-mouse-move-object").each(function (i, el) {
      jQuery("body").on("mousemove", function (e) {
        jQuery(el)[0].style.WebkitTransform =
          "translate3d(" +
          "-" +
          ((e.pageX - jQuery(this).offset().left) / jQuery(el).width()) * 10 +
          "px, 0, 0)";
      });
    });
  }
  ...
  jQuery(".lte-mouse-move .elementor-column-wrap")
    .on("mouseover", function () {
      ...
      if (jQuery(this).css("background-size") != "cover") {
        jQuery(this)[0].style.setProperty(
          "background-size",
          parseInt(jQuery(this).data("bg-size")) + 10 + "%",
          "important"
        );
      }
    })
    ...
    .on("mousemove", function (e) {
      if (jQuery(this).css("background-size") != "cover") {
        jQuery(this)[0].style.setProperty(
          "background-position",
          ((e.pageX - jQuery(this).offset().left) / jQuery(this).width()) *
            100 +
            "% " +
            ((e.pageY - jQuery(this).offset().top) / jQuery(this).height()) *
              100 +
            "%",
          "important"
        );
...
function initSearch() {
  let searchHandler = function (event) {
    if (
      jQuery(event.target).is(
        ".lte-top-search-wrapper, .lte-top-search-wrapper *"
      )
    )
      return;
  ...
  jQuery("#lte-top-search-ico-mobile").on("click", function () {
    if (search_soruce == "woocommerce") {
      window.location =
        search_href + "?s=" + jQuery(this).next().val() + "&post_type=product";
    } else {
  ...
  jQuery(".lte-top-search-wrapper input").keypress(function (e) {
    if (e.which == 13) {
      if (search_soruce == "woocommerce") {
        window.location =
          search_href + "?s=" + jQuery(this).val() + "&post_type=product";
      } else {
...
function initParallax() {
  ...
  jQuery(
    "section.lte-parallax-yes, div.lte-parallax-yes > .elementor-column-wrap"
  ).each(function (i, el) {
    var val = jQuery(el)
      .attr("class")
      .match(/lte-bg-parallax-value-(\S+)/);
  ...
  if (jQuery(".lte-parallax-slider-sc").length) {
    var parallaxInstance = new Parallax(
      document.getElementById("lte-parallax-slider-sc-wrap")
    );
  }
...
function initStyles() {
  jQuery(
    "form:not(.checkout, .woocommerce-shipping-calculator) select:not(#rating), .wpcf7-form select:not(#rating), .lte-wc-order select, aside select, .lte-footer-widget-area select"
  ).wrap('<div class="select-wrap"></div>');

  jQuery(".wpcf7-checkbox").parent().addClass("margin-none");

  jQuery('input[type="submit"], button[type="submit"]')
    .not(".lte-btn")
    .addClass("lte-btn");
  ...
  // WooCommerce styles
  jQuery(".woocommerce .button")
    .addClass("lte-btn btn-main")
    .removeClass("button");
  jQuery(".woocommerce-message .lte-btn, .woocommerce-info .lte-btn").addClass(
    "btn-xs"
  );
  jQuery(".woocommerce .price_slider_amount .lte-btn")
    .removeClass("btn-black color-hover-white")
    .addClass("btn btn-main btn-xs");
  jQuery(".woocommerce .checkout-button")
    .removeClass("btn-black color-hover-white")
    .addClass("btn btn-main btn-xs");
  jQuery("button.single_add_to_cart_button")
    .removeClass("btn-xs color-hover-white")
    .addClass("color-hover-main");
  jQuery(".woocommerce .coupon .lte-btn")
    .removeClass("color-hover-white")
    .addClass("color-hover-main");
  jQuery(".woocommerce .product .wc-label-new")
    .closest(".product")
    .addClass("lte-wc-new");
  jQuery(".widget_product_search button").removeClass("lte-btn btn-xs");
  jQuery(".woocommerce .product_meta span").each(function (i, el) {
    jQuery(el).html(
      jQuery(el).html().replace(/\,/gi, '<span class="lte-coma">,</span>')
    );
  });
...
*/
  jQuery(".woocommerce *:not(.lte-btn-wrap) > .lte-btn").wrap(
    '<span class="lte-btn-wrap"></span'
  );

  jQuery(".container .wpcf7-submit")
    .removeClass("btn-xs")
    .wrap('<span class="lte-btn-wrap"></span');

  jQuery(".blog-post .nav-links > a").wrapInner("<span></span>");
  jQuery('.blog-post .nav-links > a[rel="next"]').wrap(
    '<span class="next"></span>'
  );
  jQuery('.blog-post .nav-links > a[rel="prev"]').wrap(
  ...
  var overlays = jQuery('*[class*="lte-overlay-wrapper-"]').each(function (
    i,
    el
  ) {
    ...
    if (jQuery(this).hasClass("elementor-column")) {
      jQuery(el)
        ...
        .prepend(
          '<div class="lte-background-overlay lte-overlay-' +
            overlay[1] +
            '"></div>'
        );
    } else {
...
setTimeout(function () {
  if (
    ...
    jQuery("body").hasClass("paceloader-enabled")
  ) {
    Pace.stop();
  }
}, 3000);

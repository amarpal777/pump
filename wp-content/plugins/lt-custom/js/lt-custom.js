...
jQuery(".lt-custom-popup").on(
  ...
  function (el) {
    ...
    document.documentElement.style.setProperty(
      "--main-darker",
      ltxColorLuminance(bodyStyles.getPropertyValue("--main"), -0.05)
    );
  }
);
...
jQuery(document).on("ready", function () {
  jQuery(".lt-custom-popup").append(
    '<div class="ltx-color-selector lt-custom-main" id="ltx-color-1" data-field="main"><div id="ltx-color-1-val"></div></div>'
  );
  jQuery(".lt-custom-popup").append(
    '<div class="ltx-color-selector lt-custom-second" id="ltx-color-4" data-field="second"><div id="ltx-color-4-val"></div></div>'
  );
  jQuery(".lt-custom-popup").append(
    '<div class="ltx-color-selector" id="ltx-color-3" data-field="gray"><div id="ltx-color-3-val"></div></div>'
  );
  jQuery(".lt-custom-popup").append(
    '<div class="ltx-color-selector" id="ltx-color-2" data-field="black"><div id="ltx-color-2-val"></div></div>'
  );
  /*
...
*/
  jQuery(".ltx-color-selector").each(function (i, el) {
    ...
    if (
      jQuery(el).hasClass("lt-custom-second") &&
      jQuery.trim(bodyStyles.getPropertyValue("--main").toLowerCase()) ==
        color.toLowerCase()
    ) {
      jQuery(el).hide();
    }
    ...
    jQuery("#" + elId).ColorPicker({
      ...
      onChange: function (hsb, hex, rgb) {
        ...
        document.documentElement.style.setProperty(
          "--main-darker",
          ltxColorLuminance(bodyStyles.getPropertyValue("--main"), -0.15)
        );
        document.documentElement.style.setProperty(
          "--black-text",
          ltxColorLuminance(bodyStyles.getPropertyValue("--black"), 0.25)
        );
      },
    });
  });
  ...
  jQuery(".lt-custom-popup").prepend(

...
jQuery(window).on("elementor/frontend/init", function () {
  elementorFrontend.hooks.addAction(
    "frontend/element_ready/lte-product-categories.default",
    initSwiperWrappers
  );
  elementorFrontend.hooks.addAction(
    "frontend/element_ready/lte-products.default",
    initSwiperWrappers
  );
  elementorFrontend.hooks.addAction(
    "frontend/element_ready/lte-testimonials.default",
    initSwiperWrappers
  );
  elementorFrontend.hooks.addAction(
    "frontend/element_ready/lte-zoomslider.default",
    initSwiperWrappers
  );
  elementorFrontend.hooks.addAction(
    "frontend/element_ready/lte-gallery.default",
    initSwiperWrappers
  );
  elementorFrontend.hooks.addAction(
    "frontend/element_ready/lte-team.default",
    initSwiperWrappers
  );

  elementorFrontend.hooks.addAction(
    "frontend/element_ready/lte-rental-search.default",
    initCf7Styles
  );
  elementorFrontend.hooks.addAction(
    "frontend/element_ready/lte-cf7.default",
    initCf7Styles
  );

  elementorFrontend.hooks.addAction(
    "frontend/element_ready/accordion",
    function () {
      alert("1");
    }
  );

  jQuery(".elementor-tab-title.elementor-active").removeClass(
    "elementor-active"
  );
});
  ...
  jQuery(lteSliders).each(function (i, el) {
    ...
    if (container.data("arrows")) {
      var arrows_html =
        '<div class="' +
        id +
        "-arrows lte-arrows lte-arrows-" +
        container.data("arrows") +
        '"><a href="#" class="lte-arrow-left"></a><a href="#" class="lte-arrow-right"></a></div>';

      if (
        container.data("arrows") == "right" ||
        container.data("arrows") == "sides-outside" ||
        container.data("arrows") == "sides-small"
      ) {
        jQuery(container).after(arrows_html);
      } else if (container.data("arrows") != "custom") {
    ...
    if (
      ...
      container.data("pagination") == "bullets"
    ) {
      pagination = {
      ...
      container.data("pagination") &&
      container.data("pagination") == "fraction"
    ) {
      pagination = {
        el: ".swiper-pagination",
        type: "fraction",
      ...
      container.data("pagination") &&
      container.data("pagination") == "custom"
    ) {
      pagination = {
        ...
        renderBullet: function (index, className) {
          ...
          return (
            ...
            " " +
            pages[index]["cats"] +
            '"><span class="lte-img"><img src="' +
            pages[index]["image"] +
            '" alt="' +
            pages[index]["header"] +
            '"></span><span class="lte-title">' +
            pages[index]["header"] +
            "</span></span>"
          );
        },
...
*/
      on: {
        init: function () {
          ...
          jQuery(".swiper-slide").removeClass(
            "swiper-slide-nth-prev-2 swiper-slide-nth-next-2"
          );
          jQuery('.swiper-slide[data-swiper-slide-index="' + realIndex + '"]')
            .prev()
            .prev()
    ...
    if (
      container.data("autoplay") > 0 &&
      container.data("autoplay-interaction") === 1
    ) {
  ...
  jQuery(container).each(function (i, el) {
    ...
    if (tabs.length) {
          ...
          if (filter === 0) {
            ...
            wrapper
              ...
              .fadeOut(300, function () {
                ...
                setTimeout(function () {
                  wrapper
                    .find(".lte-filter-item.lte-filter-id-" + filter)
                    .addClass("show-item")
                    .fadeIn(300, function () {
      ...
      if (firstBtn.data("filter") != 0) {
        ...
        wrapper
          .find(
            ".lte-filter-item.lte-filter-id-" + firstBtn.data("filter") + ""
          )
          .addClass("show-item")
          .show();
        ...
        wrapper
          ...
          .css("min-height", container.height());
      }
...
function initCf7Styles() {
  jQuery("form.wpcf7-form select")
    .parent(":not(.select-wrap)")
    .find("select")
